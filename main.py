...
pos_cards = [
    ...
    upgrade.frr_upgrade_card,
    upgrade.bsp_upgrade_card,
    upgrade.heal_card,
...
DS_MaxWave = font.render(
    "Highest round reached: " + str(S_MaxWave), True, (255, 255, 255)
)
S_MaxKills = HSHM["max_kills"]
DS_MaxKills = font.render(
    "Most kills in a game: " + str(S_MaxKills), True, (255, 255, 255)
)
...
while True:
    ...
    while state == "fight":
        ...
        bullet_hits = pygame.sprite.groupcollide(
            bullet_group, enemy_group, False, False
        )
        ...
        if enemy_group.sprites():
            for i in enemy_group.sprites()[:]:
                if i.HP <= 0:
                    kills += 1
                    kills_display_surf = font.render(
                        "Kills: " + str(kills), True, (255, 255, 255)
                    )
                    i.die(gibgroup)
                    enemy_group.remove(i)
        # spawn new enemy
